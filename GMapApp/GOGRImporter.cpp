...
namespace gmap{
	...
	//´ÓOGR×ª»»Variant
	_variant_t GetOGRFieldValue(OGRFeature* feature,int index,OGRFieldType dt)
	{
		switch(dt)
		{
		...
		case OFTString:return _variant_t(feature->GetFieldAsString(index));
		case OFTDate:
		case OFTTime:
		case OFTDateTime:			
			{
	...
	GVectorDataLayer* GOGRImporter::Import(const wstring& name,OGRLayer* pLayer,GVectorDataSource* pDataSource,IProgressUI* pUI)
	{
		...
		while((pFeature=pLayer->GetNextFeature())!=NULL)
		{
			...
			for(u32 iField=0;iField<nFieldCount;iField++)
			{
				_variant_t val2 = GetOGRFieldValue(pFeature,fieldIndices[iField],fieldTypes[iField]);
